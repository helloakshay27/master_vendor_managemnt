...
export const VendorSectionSelector = ({ onSelectionChange }) => {
  ...
  useEffect(() => {
    const allSections = Object.values(VENDOR_SECTIONS).flatMap(category => 
  ...
  const selectAllForCategory = (categoryKey, selected) => {
    const category = VENDOR_SECTIONS[categoryKey];
  ...
  const getCategorySelectionState = (categoryKey) => {
    const category = VENDOR_SECTIONS[categoryKey];
    const categoryIds = category.options.map(option => option.id);
    const selectedCount = categoryIds.filter(id => selectedSections.includes(id)).length;
  ...
  const totalSections = Object.values(VENDOR_SECTIONS).reduce((acc, cat) => acc + cat.options.length, 0);
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        ...
        <span>Customize Dashboard ({selectedSections.length}/{totalSections})</span>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{
                ...
                color: '#6b7280',
                margin: 0
              }}>Choose analytics from different modules</p>
            </div>
            ...
            <div style={{
              ...
              {Object.entries(VENDOR_SECTIONS).map(([categoryKey, category], index) => {
                ...
                return (
                  <div key={categoryKey}>
                    <div style={{ padding: '16px' }}>
                      <div style={{
                        ...
                        <input
                          ...
                          onChange={(e) => selectAllForCategory(categoryKey, e.target.checked)}
                          style={{
                            width: '16px',
                            height: '16px',
                            cursor: 'pointer',
                    ...
                    {index < Object.entries(VENDOR_SECTIONS).length - 1 && (
                      <div style={{
                        height: '1px',
