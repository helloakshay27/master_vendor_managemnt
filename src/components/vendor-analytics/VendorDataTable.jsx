...
export const VendorDataTable = ({ title, data, columns, onDownload, className = "" }) => {
      ...
      <div className={`card go-shadow bg-white rounded-lg ${className}`}>
        <div className="vendor-card-header">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            {onDownload && (
              <Download
                style={{ width: '20px', height: '20px', cursor: 'pointer', color: '#6b7280' }}
                onClick={onDownload}
              />
            )}
          </div>
      ...
      <div className="vendor-card-header" style={{ flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
          ...
          {onDownload && (
            <Download
              data-no-drag="true"
              style={{ width: '20px', height: '20px', cursor: 'pointer', color: '#6b7280', pointerEvents: 'auto' }}
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                onDownload();
            ...
            <tbody style={{ backgroundColor: 'white' }}>
              {data.map((row, rowIndex) => {
                ...
                return (
                  <tr
                    key={rowIndex}
                    style={isTotal ? { backgroundColor: '#f3f4f6', fontWeight: 600 } : {}}
                  >
                    {columns.map((column, colIndex) => (
                      ...
                      >
                        {column.render ? column.render(row[column.key], row) : (row[column.key] !== undefined && row[column.key] !== '' ? row[column.key] : '')}
                      </td>
                    ))}
                  </tr>
                );
