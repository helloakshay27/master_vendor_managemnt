...
const RekycDetail = () => {
  ...
  useEffect(() => {
    if (
      showReturnFiling &&
      (gstr1Details.length === 0 || gstr3bDetails.length === 0)
    ) {
  ...
  const getBasicInfoValue = (fieldName, defaultValue = "-") => {
    ...
    const basicInfoSection = rekycData.sections.find(
      (s) => s.title === "Basic Information"
    );
    if (!basicInfoSection) return defaultValue;
    const item = basicInfoSection.items.find(
      (i) =>
    ...
    <div className="website-content">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="d-flex justify-content-end mb-3 mx-4 mt-4">
          ...
          <button
            type="button"
            onClick={fetchSapLogs}
            className="purple-btn1 rounded-3"
          >
            <span>SAP Logs</span>
          </button>
        </div>
          ...
          <div className="card-body mt-0 pt-0 pb-1">
            <div className="row header-labels mt-3 text-center">
              ...
              <div className="col-3">
                ...
                <div className="checkbox-group mt-1 justify-content-center"></div>
              </div>
            </div>
            ...
            {(() => {
              ...
              groupOrder.forEach((groupName) => {
                ...
                let items = allItems.filter((item) => {
                  ...
                  const originalSectionLower =
                    item.originalSection?.toLowerCase() || "";

                  // Match by field name or if it belongs to an MSME related section
                  const isMatch =
                    groupFields[fieldNameLower] ||
                    (groupName === "Msme Information" &&
                      (fieldNameLower.includes("msme") ||
                        originalSectionLower.includes("msme")));
                ...
                if (items.length > 0) {
                  ...
                  if (groupName === "Msme Information") {
                    items = [
                      ...items.filter(
                        (i) =>
                          !i.field_name?.toLowerCase().includes("attachment") &&
                          !i.field_name?.toLowerCase().includes("certificate")
                      ),
                      ...items.filter(
                        (i) =>
                          i.field_name?.toLowerCase().includes("attachment") ||
              ...
              const bankSection = apiSections.find(
                (s) => s.title === "Bank Details"
              );
              if (
                bankSection &&
                bankSection.items &&
                bankSection.items.length > 0
              ) {
                renderedSections.push({
                  title: "Bank Details",
                  items: bankSection.items,
                  isTable: true,
                });
              }
              ...
              apiSections.forEach((s) => {
                ...
                if (!excludedTitles.includes(titleLower)) {
                  ...
                  const unusedItems = s.items.filter(
                    (item) =>
                      !usedItemIds.has(item.id) && item.status !== "info"
                  );
                  ...
                  const specialItems = s.items.filter(
                    (item) =>
                      !usedItemIds.has(item.id) && item.status !== "info"
                  );
                  if (specialItems.length > 0) {
                    const existing = renderedSections.find(
                      (rs) => rs.title === "Addresses"
                    );
                    if (existing) {
                  ...
                  <div key={sectionName} className="mt-1">
                    ...
                    {sectionName === "GSTIN Information" && (
                      <div className="px-2 mb-2">
                        <button
                          className="purple-btn2 btn-sm mb-0"
                    ...
                    {isTableSection ? (
                      <div className="tbl-container table-responsive overflow-x-auto px-2">
                        {sectionRequests.map((req, reqIdx) => {
                          ...
                          const isDirector =
                            sectionName === "Directors Information";
                          ...
                          return (
                            <div key={req.id || reqIdx} className="mb-4">
                              ...
                              <div
                                className="row align-items-center mb-0 py-1 px-3 mx-0"
                                style={{
                                  backgroundColor: "#e0f1f7",
                                  minHeight: "40px",
                                  border: "1px solid #b3e5fc",
                                }}
                              >
                                <div
                                  className="col-4 text-start fw-bold"
                                  style={{ fontSize: "12px" }}
                                >
                                  {req.field_name || sectionName}
                                </div>
                                <div
                                  className="col-2 text-center"
                                  style={{ fontSize: "11px", color: "#555" }}
                                >
                                  Added By {req.added_by || "Vendor"}
                                </div>
                                <div
                                  className="col-2 text-center"
                                  style={{ fontSize: "11px", color: "#555" }}
                                  ...
                                  <tr>
                                    {isBank && (
                                      <>
                                        ...
                                        >
                                          {req.attachment_url ? (
                                            <a
                                              href={`${baseURL}${req.attachment_url}`}
                                              target="_blank"
                                              rel="noreferrer"
                                    ...
                                    {isFactory && (
                                      <>
                                        ...
                                        >
                                          {snapshot.tel_number ||
                                            snapshot.telephone_no ||
                                            snapshot.factoryTelephone ||
                                            "-"}
                                        </td>
                                        <td
                                          style={{
                                        ...
                                        >
                                          {snapshot.attachment?.download_url ||
                                          snapshot.factoryAttachment
                                            ?.attachment_url ? (
                                            <a
                                              href={
                                                snapshot.attachment
                                                  ?.download_url ||
                                                `${baseURL}${snapshot.factoryAttachment?.attachment_url}`
                                              }
                                              target="_blank"
                                              rel="noreferrer"
                                              className="d-flex align-items-center justify-content-center"
                                    ...
                                    {isMajor && (
                                      <>
                                        ...
                                        >
                                          {snapshot.years_of_association ||
                                            snapshot.year_of_association ||
                                            "-"}
                                        </td>
                                        <td
                                          style={{
                                        ...
                                        >
                                          {snapshot.service_provided_from ||
                                            "-"}
                                        </td>
                                        <td
                                          style={{
                        ...
                        {sectionRequests.map((req) => (
                          <React.Fragment key={`${req.id}-${req.field_name}`}>
                            <div className="rekyc-row row align-items-center text-center">
                              <div className="col-2 text-start">
                                <label>
                                  {req.field_name || req.record_type}
                                </label>
                              </div>
                              <div className="col-2">
                                ...
                                <span className="d-flex align-items-center justify-content-center">
                                  ...
                                  req.record_type === "Attachfile" ? (
                                    <div className="attachment-link d-flex align-items-center">
                                      ...
                                      {(req.record_snapshot?.download_url ||
                                        req.attachment_url) && (
                                        <a
                                          className="ms-1"
                                          href={
                                            req.record_snapshot?.download_url ||
                                            `${baseURL}${req.attachment_url}`
                                          }
                                          target="_blank"
                                          rel="noreferrer"
                                          style={{ textDecoration: "none" }}
                                ...
                                >
                                  <div className="row">
                                    {[
                                      {
                                        label: "Address",
                                        value: req.new_address.address,
                                      },
                                      {
                                        label: "Address Line 2",
                                        value: req.new_address.address_line_2,
                                      },
                                      {
                                        label: "Address Line 3",
                                        value: req.new_address.address_line_3,
                                      },
                                      {
                                        label: "Address Line 4",
                                        value: req.new_address.address_line_4,
                                      },
                                      {
                                        label: "Address Line 5",
                                        value: req.new_address.address_line_5,
                                      },
                                      {
                                        label: "Country",
                                        value: req.new_address.country,
                                      },
                                      {
                                        label: "State",
                                        value: req.new_address.state,
                                      },
                                      {
                                        label: "City",
                                        value: req.new_address.city,
                                      },
                                      {
                                        label: "Pin Code",
                                        value: req.new_address.pin_code,
                                      },
                                      {
                                        label: "Telephone Phone No.",
                                        value: req.new_address.telephone_no,
                                      },
                                      {
                                        label: "Mobile Number",
                                        value: req.new_address.mobile_number,
                                      },
                                      {
                                        label: "Email ID",
                                        value: req.new_address.email,
                                      },
                                    ].map((field, idx) => (
                                      <div
                                        key={idx}
                                        className="col-lg-6 col-md-6 col-sm-12 row mb-1 px-3"
                                      >
                                        <div className="col-5">
                                          <label
                                            style={{
          ...
          <div className="card-body mt-0 pt-2">
            <div className="tbl-container table-responsive">
              <table className="table w-100 mb-0">
                ...
                <tbody>
                  {(rekycData?.audit_logs || rekycData?.auditLogs || []).map(
                    (log, index) => (
                      <tr key={log.id}>
      ...
      >
        <div className="mb-3">
          ...
          >
            {editModal.dropdownKey ? "Select" : "Enter"}{" "}
            {editModal.request?.field_name}
          </label>
          {editModal.dropdownKey === "msme_hardcoded" ? (
            ...
            >
              ...
              {dropdowns[editModal.dropdownKey]
                .filter((option) => {
                  if (
                    editModal.dropdownKey === "schema_group" ||
                    editModal.dropdownKey === "gst_classification"
                  ) {
                    return (
                    ...
                    editModal.dropdownKey === "purchasing_organization"
                  ) {
                    return (
                      option.purchase_org_name !== null &&
                      option.purchase_org_name !== undefined
                    );
                  }
                ...
                .map((option, index) => {
                  ...
                  if (editModal.dropdownKey === "schema_group") {
                    optionLabel = option.code;
                  } else if (editModal.dropdownKey === "gst_classification") {
      ...
      {showReturnFiling && (
        <DynamicModalBox
          ...
          size="lg"
          title={
            <span style={{ color: "#de7008" }}>Return Filing Details</span>
          }
        >
          <div className="row mt-2 px-2">
            <div className="col-12">
